...
namespace Infrastructure.Services.Summarizer
{
    public class AzureSummarizerService : IChunkProcessorService, IMergeProcessorService
    {
        ...
        /// </summary>
        public async Task<SummarizerResponse> MergeSummariesAsync(
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            {
                IntentType.Summarize => SummarizerPromptBuilder.BuildMergePrompt(partialSummaries, language),
                IntentType.Analyze => SummarizerPromptBuilder.BuildAnalyzePrompt(originalLogs, dateRange, language),
                IntentType.Report => SummarizerPromptBuilder.BuildReportPrompt(partialSummaries, dateRange, language),
        ...
        private async Task<List<SummarizerResponse>> ProcessInBatchesAsync(IChatClient chatClient, List<LogChunk> chunks, CancellationToken ct)
        {
            ...
            var promptTemplateText = SummarizerPromptBuilder.GetBatchChunkPromptTemplate(InputLanguage.English);
            ...
            if (chunks.Any())
            {
                ...
                foreach (var chunk in chunks)
                {
                    var requestItem = new BatchSummarizeRequestItem {
        ...
        private async Task<SummarizerResponse?> SummarizeSingleChunkAsync(IChatClient chatClient, LogChunk chunk, CancellationToken ct)
        {
            try
            {
                var singleChunkBatch = new List<LogChunk> { chunk };

                var results = await ProcessSingleBatchAsync(chatClient, singleChunkBatch, ct);
